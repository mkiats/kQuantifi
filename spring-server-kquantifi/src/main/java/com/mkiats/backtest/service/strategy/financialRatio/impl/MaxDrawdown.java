...
public class MaxDrawdown implements FinancialRatioStrategy {
	...
	public FinancialRatioOutput computeRatio(
		...
		for (TimeValue theTimeValue : investmentOutput.getChartData()) {
			...
			//			System.out.println(
			//				"Current time: " + curTimestamp + "Current price: " + curPrice
			//			);
			...
			if (curPrice > peak) {
				...
				financialRatioOutput
					.getMaxDrawdown()
					.addTimeValue(curTimestamp, 100.0, 0.0);
			} else {
				...
				financialRatioOutput
					...
					.addTimeValue(
						curTimestamp,
						calculateDrawdownValue(curPrice, peak),
						0.0
					);
			}
		}
	...
	public double calculateDrawdownValue(double curPrice, double peak) {
		BigDecimal rawDrawdown = new BigDecimal(
			((peak - curPrice) * 100) / peak
		);
		BigDecimal roundedDrawdown = rawDrawdown.setScale(
			2,
